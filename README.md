...
```bash
docker run --rm  pgbackrest:2.45 pgbackrest help
```

### Injecting inside

```bash
docker run --rm -it pgbackrest:2.45 bash
...
```bash
make build_version TAG=2.45
```

```bash
make build_version_alpine TAG=2.45
```

or

```bash
docker build -f Dockerfile --build-arg BACKREST_VERSION=2.45 --build-arg BACKREST_COMPLETION_VERSION=v0.9 -t pgbackrest:2.45 .
```
