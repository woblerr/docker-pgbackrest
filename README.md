...
```bash
docker run --rm -it pgbackrest:2.55.1 bash
...
```bash
make build_version TAG=2.55.1
```

```bash
make build_version_alpine TAG=2.55.1
```

or

```bash
docker build -f Dockerfile --build-arg BACKREST_VERSION=2.55.1 --build-arg BACKREST_COMPLETION_VERSION=v0.10 -t pgbackrest:2.55.1 .
```

```bash
docker build -f Dockerfile.alpine --build-arg BACKREST_VERSION=2.55.1 --build-arg BACKREST_COMPLETION_VERSION=v0.10 -t pgbackrest:2.55.1-alpine .
```
