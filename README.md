...
```bash
docker run --rm  pgbackrest:2.42 pgbackrest help
```

### Injecting inside

```bash
docker run --rm -it pgbackrest:2.42 bash
...
```bash
make build_version TAG=2.42
```

```bash
make build_version_alpine TAG=2.42
```

or

```bash
docker build -f Dockerfile --build-arg BACKREST_VERSION=2.42 --build-arg BACKREST_COMPLETION_VERSION=v0.8 -t pgbackrest:2.42 .
```

```bash
docker build -f Dockerfile.alpine --build-arg BACKREST_VERSION=2.42 --build-arg BACKREST_COMPLETION_VERSION=v0.8 -t pgbackrest:2.42-alpine .
```
